...
/********************************************************
 * Template Engine
 * Class for templatating Zeroappz Framework
  ********************************************************
 * @package Template Engine
 * @author Devendra Katariya
 * @copyright 20016-17 Zeroappz
 ********************************************************/
class HtmlTemplate
{
	...
	* ***********************************************************************/
	function CreatePageEcho ()
	{
		...
		$this->html = $this->convert_html($this->html);
		echo $this->html;
	}

	private function convert_html(){
        ...
        if(isset($_SESSION['user']['id']))
        {
            ...
            $this->html = str_replace ('{USERTYPE}', $_SESSION['user']['user_type'], $this->html);
        ...
        else
        {
            ...
            $this->html = str_replace ('{USERTYPE}', '', $this->html);
		...
		return $this->convert_if($this->html);
	}

	private function convert_if($html)
	{
		// convert only elseif
		$matches = array();
		preg_match_all('/ELSEIF\((.*?)\)\{/s', $html, $matches);

		if(count($matches['0']) != 0)
		{
			foreach ($matches['0'] as $key => $value)
			{
				$html = str_replace($value, '<?php } elseif('.trim($matches[1][$key]).') { ?>', $html);
			}
		}

		// convert only if()
		$matches = array();
		preg_match_all('/IF\((.*?)\)\{/s', $html, $matches);

		if(count($matches['0']) != 0)
		{
			foreach ($matches['0'] as $key => $value)
			{
				$html = str_replace($value, '<?php if('.trim($matches[1][$key]).'){ ?>', $html);
			}
		}
		...
		// convert only else
		$matches = array();
		preg_match_all('/\{ELSE\}/s', $html, $matches);

		if(count($matches['0']) != 0)
		{
			foreach ($matches['0'] as $key => $value)
			{
				$html = str_replace($value, '<?php } else { ?>', $html);
			}
		}
		...
		if(count($matches['0']) != 0)
		{
			foreach ($matches['0'] as $key => $value)
			{
				$html = str_replace($value, '<?php } ?>', $html);
			}
		}
