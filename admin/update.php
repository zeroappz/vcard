...
//Check For An Update
$getVersions = file_get_contents('https://bylancer.com/api/quickvcard-release-versions.php') or die('ERROR');
if ($getVersions != '') {
    ...
    $versionList = explode("\n", $getVersions);
                    foreach ($versionList as $aV) {
                        if ($aV > $config['version']) {
                            ...
                            if (!is_file($update_dir . 'QUICKVCARD-CMS-' . $aV . '.zip')) {

                                if (!is_dir($update_dir)) mkdir($update_dir);
                ?>
                                <form id="upload" method="post" action="upload.php" enctype="multipart/form-data">
                                    <div id="drop">
                                        Drop Here <br>QUICKVCARD-CMS-<?php echo $aV ?>.zip
                                        <a>Browse</a>
                ...
                <?php
                    break;
                            ...
                            if (isset($_GET['doUpdate']) && $_GET['doUpdate'] == true) {
                                ...
                                $zipHandle = zip_open($update_dir . 'QUICKVCARD-CMS-' . $aV . '.zip');
                                ...
                                // Close the file
                                fclose($handle);
                                unlink($update_dir . 'QUICKVCARD-CMS-' . $aV . '.zip');
                                //unlink($update_dir);
