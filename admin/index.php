...
$total_active_item = ORM::for_table($config['db']['pre'].'vcards')->count();
    ...
    <div class="container-fluid p-y-md">
                ...
                <a class="card" href="#">
                    <div class="card-block clearfix">
                        <div class="pull-right">
                            <p class="h6 text-muted m-t-0 m-b-xs">Total VCards</p>
        ...
        <div class="row">
            <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                <div class="card">
                    <div class="card-header">
                        <h4>Recent 5 VCards</h4>
                    </div>
                    <div class="card-block">
                        <?php
                        $getItem = ORM::for_table($config['db']['pre'].'vcards')->limit(5)->orderByDesc('id')->find_many();
                        foreach ($getItem as $ads) {
                            $ad_id          = $ads['id'];
                            $ad_title       = $ads['title'];
                            $ad_location    = $ads['subtitle'];
                            ...
                            <div class="pro-list">
                                <div class="pro-img p-r-10">
                                    <a href="#">
                                        <img src="../storage/cards/logo/<?php echo $ad_pic; ?>" alt="<?php echo $ad_title ?>" style="width:75px; height: 75px;">
                                    </a>
                                </div>
                                <div class="pro-detail">
                                    <h5 class="m-t-0 m-b-5">
                                        <a href="<?php echo $config['site_url'].$ads['slug'] ?>"><?php echo $ad_title; ?></a>
                                    </h5>
                        ...
                        <div class="text-right">
                            <a href="vcards.php" class="btn btn-sm btn-rounded btn-info m-t-10">View All</a>
                        </div>
                    </div>
                </div>
            </div>
...
<?php
$today = date('Y-m-d');
$todayPost = ORM::for_table($config['db']['pre'].'vcards')->where_like('created_at', ''.$today.'%')->count();
...
for ($x = 0; $x <= 5; $x++) {
    ...
    $rows = ORM::for_table($config['db']['pre'].'vcards')->where_like('created_at', ''.$minusValue2.'%')->count();
    $chartPostValue .= ",'".$rows."'";
