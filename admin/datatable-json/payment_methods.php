...
foreach ($queryRecords as $row) {
    ...
    if($row['payment_install'] == 1) {
        ...
        $install_button = '<a href="#" class="btn btn-xs btn-warning uninstall-payment" data-ajax-action="uninstallPayment" data-folder="'.$folder.'"><i class="fa fa-close"></i> Uninstall</a>';
    }
    else{
        $install_button = '<a href="#" class="btn btn-xs btn-success install-payment" data-ajax-action="installPayment" data-folder="'.$folder.'"><i class="fa fa-download"></i> Install</a>';
    }
    ...
    $row1 = '<td><img src="'.$config['site_url'].'admin/assets/images/payments/'.$folder.'.png" height="40px"/></td>';
    $row2 = '<td>'.$name.'</td>';
    $row3 = '<td>'.$install.'</td>';
